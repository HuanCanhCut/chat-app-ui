...
const InputMessage: React.FC<InputMessageProps> = () => {
    ...
    if (currentMember?.deleted_at || !currentMember) {
        return (
            <div className="flex flex-col gap-2 border-t border-light px-4 py-2 text-center text-black/90 dark:border-darkGray dark:text-gray-300">
    ...
    if (conversation?.data.block_conversation) {
        ...
        if (blocker.id === currentUser?.data.id) {
            return (
                <div className="flex flex-col gap-2 border-t border-light px-4 py-4 text-center text-black/90 dark:border-darkGray dark:text-[#e2e5e9]">
                    <p className="text-center text-[13px] font-medium">
                        Bạn đã chặn tin nhắn của{' '}
                        {(memberMap && memberMap[blocker.id]?.nickname) ||
                            (memberMap && memberMap[blocker.id]?.user.full_name)}
                    </p>
                    <p className="text-xs text-[#b2b5b8]">Các bạn không thể nhắn tin cho nhau trong đoạn chat này</p>
