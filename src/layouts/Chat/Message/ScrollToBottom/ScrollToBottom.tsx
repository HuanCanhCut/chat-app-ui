...
const ScrollToBottom = ({ offsetRange, scrollableRef, messageRefs, PER_PAGE, setOffsetRange }: ScrollToBottomProps) => {
    ...
    const handleScrollToBottom = useCallback(
        async (block: ScrollLogicalPosition = 'center') => {
            if (offsetRange.start === 0) {
                if (messageRefs.current) {
                    ...
                    for (let i = key.length - 1; i >= 0; i--) {
                        if (messageRefs.current[key[i]]) {
                            messageRefs.current[key[i]].scrollIntoView({ behavior: 'smooth', block })
                            break
                        }
                    }
                }
        ...
        [PER_PAGE, messageRefs, offsetRange.start, setOffsetRange, uuid],
    )
