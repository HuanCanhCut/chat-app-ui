...
export default function Button({ children, className, href, buttonType = 'primary', leftIcon, ...props }: ButtonProps) {
    ...
    const buttonIconClass = buttonType === 'icon' ? 'h-9 w-9 rounded-full bg-gray-100 dark:bg-darkGray' : ''
    const roundedClass = buttonType === 'rounded' ? 'rounded-md px-4 py-2 dark:bg-darkGray bg-gray-400' : ''
